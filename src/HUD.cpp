...
std::string game::HUD::getPlayerLabel(indie::PlayerId id) noexcept
{
    switch (indie::PlayersManager::getPlayerType(id)) {
    case indie::PlayerType::KEYBOARD:
        return fmt::format("Player {}", this->playerCount++ + 1);
    case indie::PlayerType::AI:
        return fmt::format("AI {}", this->aiCount++ + 1);
    case indie::PlayerType::NETWORK:
