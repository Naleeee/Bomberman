...
ecs::component::RenderableRelief3d::RenderableRelief3d(std::string_view reliefPath, indie::Vector size, Color color) noexcept
{
    this->valid = true;
    this->color = color;
...
void ecs::component::RenderableRelief3d::setRelief(std::string_view reliefPath, indie::Vector size, Color color) noexcept
{
    if (this->valid) {
        UnloadModel(this->model);
        UnloadTexture(this->relief);
