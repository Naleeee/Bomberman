...
void ecs::system::TransformSystem::update(ecs::Coordinator& coordinator, float elapsedTime) const noexcept
{
    for (const auto& entity : this->entities) {
        ...
        attributes.position += indie::Vector{(transform.movingX ? transform.movement.x : 0), (transform.movingY ? transform.movement.y : 0), (transform.movingZ ? transform.movement.z : 0)} * elapsedTime;
    }
}
