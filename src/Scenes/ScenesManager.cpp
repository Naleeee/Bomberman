...
graphic::ScenesManager::ScenesManager(graphic::Renderer& renderer, game::Room& room, ecs::system::CollisionSystem& collisionSystem, ecs::Coordinator& coordinator) noexcept :
    renderer(renderer), room(room), collisionSystem(collisionSystem), coordinator(coordinator)
{
}

void graphic::ScenesManager::update(float dt)
{
    if (this->lastScene) {
...
void graphic::ScenesManager::manageTransition() noexcept
{
...
void graphic::ScenesManager::initTransition() noexcept
{
...
void graphic::ScenesManager::stopTransition() noexcept
{
...
static bool checkCameraXPosition(indie::Vector cameraDestination, indie::Vector cameraMovement, indie::Vector cameraPosition)
{
...
static bool checkCameraYPosition(indie::Vector cameraDestination, indie::Vector cameraMovement, indie::Vector cameraPosition)
{
...
static bool checkCameraZPosition(indie::Vector cameraDestination, indie::Vector cameraMovement, indie::Vector cameraPosition)
{
...
bool graphic::ScenesManager::transitionIsEnded() const noexcept
{
...
void graphic::ScenesManager::drawWaitingScreen()
{
    ...
    this->coordinator.setComponent(text, ecs::component::RenderableText2d{.text = tips.at(indie::Randomizer::getRandomNumber<int>(0, tips.size() - 1)), .fontSize = 30, .color = DARKPURPLE});
