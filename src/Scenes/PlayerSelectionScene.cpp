...
graphic::PlayerSelectionScene::PlayerSelectionScene(game::Room& room, ecs::Coordinator& coordinator, std::string_view path) :
    coordinator(coordinator), title(this->coordinator.createEntity()), bomberman(this->coordinator.createEntity()), left(this->coordinator.createEntity()), right(this->coordinator.createEntity()), choose(this->coordinator.createEntity()), mapReader(path, room.getMapSize())
{
...
graphic::PlayerSelectionScene::~PlayerSelectionScene() noexcept
{
...
void graphic::PlayerSelectionScene::update(graphic::ScenesManager& scenesManager, float /*dt*/)
{
    if (IsKeyPressed(KEY_A)) {
        if (this->selectedTexture > 0) {
            audio::AudioPlayer::playSound(fmt::format("{}audio/Sounds/MenuMove.mp3", ASSETS_PATH));
            this->selectedTexture--;
            indie::PlayersManager::setPlayerTexture(1, this->textures.at(this->selectedTexture));
    ...
    if (IsKeyPressed(KEY_ENTER)) {
        ...
        indie::PlayersManager::setPlayerTexture(1, this->textures.at(this->selectedTexture));
        scenesManager.setScene<graphic::GameScene, true, true, true>(this->coordinator, std::move(this->mapReader));
    }
}

indie::Vector graphic::PlayerSelectionScene::getCameraPosition() const noexcept
{
    return indie::Vector{-30, 5, 30};
}

indie::Vector graphic::PlayerSelectionScene::getCameraLooking() const noexcept
{
    return indie::Vector{40, 0, 0};
}
