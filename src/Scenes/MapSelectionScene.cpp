...
graphic::MapSelectionScene::MapSelectionScene(ecs::Coordinator& coordinator, int type) :
...
graphic::MapSelectionScene::~MapSelectionScene() noexcept
{
...
void graphic::MapSelectionScene::update(graphic::ScenesManager& scenesManager, float /*dt*/)
{
    if (IsKeyPressed(KEY_ENTER)) {
        ...
        path.append(".json");
        if (this->type == 0) {
            scenesManager.setScene<graphic::PlayerSelectionScene, true>(this->coordinator, path);
        } else if (this->type == 1) {
            scenesManager.setScene<graphic::MultiPlayerSelectionScene, true>(this->coordinator, path);
        }
...
indie::Vector graphic::MapSelectionScene::getCameraPosition() const noexcept
{
    return indie::Vector{-20, 83, 0};
}

indie::Vector graphic::MapSelectionScene::getCameraLooking() const noexcept
{
    return indie::Vector{200, 83, 1};
}
