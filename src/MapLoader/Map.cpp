...
#include "HitboxDisplayer.hpp"
#include "MapLoader/AIPlayer.hpp"
#include "MapLoader/APowerUp.hpp"
#include "MapLoader/KeyBoardPlayer.hpp"
#include "PointDisplayer.hpp"

indie::Map::Map(ecs::Coordinator& coordinator, ecs::system::CollisionSystem& collisionSystem, const MapReader& mapReader, const Vector& mapPosition) :
    coordinator(coordinator), collisionSystem(collisionSystem)
{
    ...
    this->mapStarting = mapPosition - Vector{-((this->blockSize.x * map.size()) / 2), 0, (this->blockSize.z * map.at(0).size()) / 2};
    Vector centerZero = this->mapStarting + Vector{-(this->blockSize.x / 2), 0, this->blockSize.z / 2};
    ...
    for (int i = 0; const auto& line : map) {
        ...
        for (int y = 0; const auto& c : line) {
            ...
            auto position = centerZero + (blockSize * Vector{-static_cast<float>(i), 0, static_cast<float>(y)});
            ...
            if (infos.type == PartType::PLAYER) {
                ...
                switch (indie::PlayersManager::getPlayerType(playerId)) {
                case PlayerType::KEYBOARD:
                    this->players.emplace_back(std::make_unique<KeyBoardPlayer>(coordinator, playersInfos, indie::PlayersManager::getPlayerTexture(playerId), position, playerId, indie::PlayersManager::getPlayerKeys(playerId), *this));
                    break;
                case PlayerType::AI:
...
void indie::Map::updateBombs() noexcept
{
    for (auto it = this->bombs.begin(); it != this->bombs.end();) {
        if (it->second.shouldExplose()) {
            ...
            for (int i = 1; i <= range.up; i++) {
                ...
                part.setOnFire(this->coordinator, this->explosionsInfos.model, it->second.getPosition() + Vector{this->blockSize.x * i, 0, 0});
            }
            if (range.haveUp) {
            ...
            for (int i = 1; i <= range.left; i++) {
                ...
                part.removeDanger(id);
                part.setOnFire(this->coordinator, this->explosionsInfos.model, it->second.getPosition() - Vector{0, 0, this->blockSize.z * i});
            }
            if (range.haveLeft) {
...
bool indie::Map::putBomb(Vector position, APlayer& player) noexcept
{
    ...
    auto center = this->mapStarting + Vector{(-this->blockSize.x) * indexes.x, 0, this->blockSize.z * indexes.y} + Vector{-(this->blockSize.x / 2), 0, this->blockSize.z / 2};
...
std::array<game::PlayerStat, 4> indie::Map::getPlayersStats() const noexcept
{
...
indie::AroundMapPartStates indie::Map::getAroundParts(Vector position) const noexcept
{
...
indie::AroundMapPartTypes indie::Map::getAroundPartsType(Vector position) const noexcept
{
    auto indexes = this->convertVectorAsPosition(position);
...
// NOLINTNEXTLINE
indie::Position indie::Map::convertVectorAsPosition(Vector position) const noexcept
{
...
bool indie::Map::isCompletelyInsidePart(Vector position, BoundingBox hitbox, float modifier) const noexcept
{
    Vector delimSize = this->blockSize * modifier;
    auto indexes = this->convertVectorAsPosition(position);
    auto nearest = this->mapStarting + Vector{(-this->blockSize.x) * indexes.x, 0, this->blockSize.z * indexes.y} + Vector{-(this->blockSize.x / 2), 0, this->blockSize.z / 2};
...
indie::Vector indie::Map::getNearestPartCenter(Vector& position) const noexcept
{
    auto indexes = this->convertVectorAsPosition(position);
    Vector nearest = this->mapStarting + Vector{(-this->blockSize.x) * indexes.x, 0, this->blockSize.z * indexes.y} + Vector{-(this->blockSize.x / 2), 0, this->blockSize.z / 2};

    return (nearest);
}

indie::Range indie::Map::getRange(Position position, unsigned int maximumRange) noexcept
{
    Range range;
    bool up = true;
    bool down = true;
...
bool indie::Map::isEnded() const noexcept
{
    int alivePlayers = 0;
...
std::vector<std::unique_ptr<indie::APlayer>> indie::Map::extractPlayers() noexcept
{
